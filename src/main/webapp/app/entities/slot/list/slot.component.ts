...
import { EntityArrayResponseType, SlotService } from '../service/slot.service';
...
export class SlotComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISlot): number => this.slotService.getSlotIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.slots = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISlot[] | null): ISlot[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.slotService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
